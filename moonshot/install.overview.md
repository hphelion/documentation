...
An HP Helion Cloud OS for Moonshot infrastructure requires advance planning and preparation by IT network administrators. The HP Helion Cloud OS for Moonshot installation 
deploys the HP Helion Cloud OS for Moonshot Operational Dashboard (used to set up, configure, and install an HP Helion Cloud infrastructure) and the HP Helion Cloud OS for 
Moonshot Administration Dashboard (used to create and manage cloud resources).

* [HP Helion Cloud OS for Moonshot Environment](#hp-cloud-os-for-moonshot-environment)
...
* [Components of the HP Helion Cloud OS for Moonshot Environment](#components-of-the-hp-cloud-os-for-moonshot-environment)

* [Next Step](#next-step)

## HP Helion Cloud OS for Moonshot Environment

The HP Helion Cloud OS for Moonshot environment provides you with: 
...
The HP Helion Cloud OS for Moonshot environment also include an installer, services, and controllers that provide simplified service delivery with automated, live 
...
<img src="media/cloudos-moonshot-install-process.png" title="HP Helion Cloud OS for Moonshot installation process" /> 

The steps shown above feature an automated process in the HP Helion Cloud OS for Moonshot Operational Dashboard. HP Helion Cloud OS for Moonshot handles all of the 
installation details for you, installing the OpenStack services and the HP Helion Cloud OS for Moonshot value-added services.

Taking a broader view, this next diagram illustrates those steps in a larger context:

<img src="media/cloudos-moonshot-install-process-expanded.png" title="HP Helion Cloud OS for Moonshot - an expanded view of the install process" /> 
...
<img src="media/cloudos-moonshot-post-install-tasks.png" title="HP Helion Cloud OS for Moonshot post-install tasks" /> 
...
Assuming a typical configuration, the overall setup process involves the following steps. The total time to complete the setup once HW is installed, configured and the hypervisor is ready is four to six hours. In this list, approximate execution times are shown here for reference, once the installation from the HP Helion Cloud OS for Moonshot ISO has begun. 
...
* Create a VM to use as an access node for installation of the HP Helion Cloud OS for Moonshot software ISO
...
## Components of the HP Helion Cloud OS for Moonshot Environment

The following diagram illustrates the HP Helion Cloud OS for Moonshot environment.

<img src="media/cloudos-moonshot-environment3.png" title="HP Helion Cloud OS for Moonshot environment" /> 
...
The list of services that can be deployed for HP Helion Cloud OS for Moonshot are:
...
* <b>Apollo</b> &mdash; An HP Helion Cloud OS for Moonshot service that provides HP Moonshot Chassis and Server Management. In addition, it helps to discover HP Moonshot Servers and allocates them for the provisioning.

* <b>Graffiti</b> &mdash; An HP Helion Cloud OS for Moonshot resource pool registry and capability tagging service. Designed to support
...
* <b>Eve</b> &mdash; An HP Helion Cloud OS for Moonshot service to provide for provisioning lifecycle
...
* <b>Focus</b> &mdash; An HP Helion Cloud OS for Moonshot service to provide for discovering, registering, versioning, and retrieving of document types necessary to describe
TOSCA-based infrastructure topologies.

### Administration Node

The HP Helion Cloud OS for Moonshot Administration Node (Admin Node) hosts the Operational Dashboard that deploys private and hybrid-only cloud infrastructure environments by network booting the managed Baremetal
servers, which are the Controller and Baremetal host. 

### Controllers

While each service can be individually deployed, HP Helion Cloud OS for Moonshot groups these services into distinct groups for ease of architectural description.

...
---
# UNIX CLI for HP Helion Cloud References for v12.12
...
You  need your Access Key Id, Secret Key and Tenant Id from the HP Helion Cloud web site to set up your account. Optionally, you can specify your own endpoint to access, but in most cases we recommend you use the default.
...
###Description
Migrates files from the deisgnated provider to the HP Helion Cloud destination. This command works similarly to `copy` except the first argument is the source account (for example, `AWS`).  The source objects may be containers, objects, or regular expressions.

...
5. Confirm that you have access to the [pypi](https://pypi.python.org/pypi) site. The following commands will verify access the site and download one of the prerequisite files as a brief test.

		ping pypi.python.org
		sudo -E pip install python-novaclient

6. If your network uses a proxy, you must define the proxy exclusion list so that proxies are not used for communicating with the services in the overcloud. To define the proxy inclusion list, enter the following command:
...
7. Run this command to prepare the installer and ensure the prerequisites are met.
		...
		sudo ./DevelopmentPlatform_Setup.sh -p {admin_user_password} -a {auth_host_ip_address} 

...
1. Register at and download the package from the [HP Helion Development Platform](https://helion.hpwsportal.com/#/Product/%7B%22productId%22%3A%221245%22%7D/Show) (HDN). 
...
5. Edit `dev-platform-installer.conf`.
	...
	1. Set the identity URL if necessary.
	 
 
7. Run this command to prepare the installer and ensure the prerequisites are met.

		sudo ./DevelopmentPlatform_Setup.sh -p {admin_user_password} -a {auth_host_ip_address} -e {location/ephemeralca-cacert.crt} install
...
9. Optionally, you can also specify the Username, and Region at this time.</br> By default the Username is *admin* and the Region is *regionOne*. 
		...
		./DevelopmentPlatform_Setup.sh -p {admin_user_password} -a {auth_host_ip_address} -e {cacert file} install

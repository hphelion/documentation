...
### Download the Cue Service {#marketplace-connect}{#cue-download}
...
## Install the DBaaS/Trove Service {#databse-install}

This section provides details on installing the HP Helion Development Platform DBaaS/Trove service.
...
The DBaaS/Trove service needs an additional network for administrative purposes. This network is currently not used in Community edition but must exist in order for Database Service to properly install.
...
The DBaaS/Trove service will be installed into the **Admin** project of the Helion OpenStack overcloud. The **Admin** project must have sufficient quotas available for the service to use. To check existing quota availability, log in to Horizon as the **Admin** user and open the **Overview** panel under the **Compute** tab.

For the Community edition, the DBaaS/Trove service requires that the Admin project have the following quotas available:
...
### Download the DBaaS/Trove Service {#database-connect}{#database-connect}
...
3. Log out from the Horizon dashboard. Log back into the Horizon dashboard as a non-admin user and click on the **DBaaS/Trove** panel under the current Project to being using Database Service.

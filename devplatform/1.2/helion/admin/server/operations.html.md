...
permalink: /helion/devplatform/als/admin/server/operations/
...
Roles are logical groups of processes (see [kato role info](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-role-info)) which can be inspected individually with [kato process
...](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-process-list) commands.

In particular, the [kato process ready](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-process-ready) command is useful for determining if the system is in a state to receive [configuration commands](/helion/devplatform/als/admin/server/configuration/#server-configuration). For example, to check that all processes for the configured roles are ready:
...
"out-of-the-box" configuration, use the [*kato node reset*](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-node-attach) command. This command has two options:
...
-   `kato node reset soft`: Resets only
    ...
    node
    setup*](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-node-attach)
    command. For example:
...
The [*Management Console*](/helion/devplatform/als/user/console/#management-console) has a Settings page that allows an administrator to monitor the server component and services, and restart or stop services as necessary.

### New Relic[](#new-relic "Permalink to this headline")

Please see [*New Relic Server Monitoring*](/helion/devplatform/als/admin/best-practices/#bestpractices-nrsysmond). New Relic can also be used to [monitor apps](/helion/devplatform/als/user/deploy/newrelic/#newrelic).
...
Console under [Users](/helion/devplatform/als/admin/console/customize/#console-users). The Management Console will prompt to create the initial admin user the first time you use it.

If you do not have access to the Management Console, create a user by logging in to the micro cloud or Core node controller via SSH or through the VM [tty console](/helion/devplatform/als/user/reference/glossary/#term-tty-console) as the *helion* user and run *helion register*:
...
and report resource utilization. See the [Best Practices](/helion/devplatform/als/admin/best-practices/#bestpractices-nagios) documentation for details.
...
Please see the [Best Practices](/helion/devplatform/als/admin/best-practices/#bestpractices-controller-migration) documentation for details on how to [export](/helion/devplatform/als/admin/best-practices/#bestpractices-migration-export) and [import](/helion/devplatform/als/admin/best-practices/#bestpractices-migration-import) your data.

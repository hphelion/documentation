...
permalink: /helion/devplatform/als/admin/server/configuration/
...
In an Application Lifecycle Service cluster, this change only happens on the node serving the Management Console pages (which could be one of [multiple Controller nodes](/helion/devplatform/als/admin/cluster/#cluster-multi-controllers)). In this case, it's best to log in to each node in the cluster to change the password
manually with the *passwd* command.

##Network Setup {#network-setup}

### Changing the Hostname {#changing-the-hostname}
You may want or need to change the hostname of the Application Lifecycle Service system, either to match a DNS record you've created or just to make the system URLs more convenient. This can be done using the [kato node rename](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref) command:

    kato node rename mynewname.example.com

This command will change the system hostname in */etc/hostname* and */etc/hosts,* as well as performing some internal configuration for Application Lifecycle Service such as generating a new server certificate for the [Management Console](/helion/devplatform/als/user/console/#management-console).
...
In a [cluster](/helion/devplatform/als/user/reference/glossary/#term-cluster), you
may also need to manually [modify the */etc/hosts file*](#server-config-etc-hosts).

### Changing IP Addresses {#changing-ip-addresses}

The Application Lifecycle Service *micro cloud* server is initially set up for [DHCP](/helion/devplatform/als/user/reference/glossary/#term-dhcp) and [multicast DNS](/helion/devplatform/als/user/reference/glossary/#term-multicast-dns). This is
...
[cluster](/helion/devplatform/als/user/reference/glossary/#term-cluster) consisting
...
PaaS*](/helion/devplatform/als/user/reference/glossary/#term-private-paas) for
...
of roles in a [cluster](/helion/devplatform/als/admin/cluster/#cluster-setup):

-   The **core** node which we conventionally call `api.helion-xxxx.local` in a micro cloud will be given its own hostname and IP address in a cluster so that you can reach it from both the [Management Console](/helion/devplatform/als/user/console/#management-console) and the command line.
...
The easiest way to configure an Application Lifecycle Service VM with a static IP address is to use the [kato op static\_ip](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-op) command.
...
If the IP address provided differs from the previous one, and the node isn't configured as a micro cloud, [kato node migrate](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-node-migrate) is run automatically.
...
**Note**
If you are setting a new static IP *after* having configured a cluster, you must reconfigure all other nodes in the cluster to use the new MBUS IP address. Run [kato node attach](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-node-attach) on all non-Core nodes. 
...
[Cluster](/helion/devplatform/als/admin/cluster/#cluster-setup) rely on finding the
...
what you should expect to see on a [micro cloud](/helion/devplatform/als/user/reference/glossary/#term-micro-cloud)
...
On a [cluster](/helion/devplatform/als/user/reference/glossary/#term-cluster)
...
The Application Lifecycle Service micro cloud uses [multicast DNS](/helion/devplatform/als/user/reference/glossary/#term-multicast-dns) to broadcast its generated hostname (e.g. `helion-xxxx.local`). This mechanism is intended for VMs running on a local machine or subnet.
...
using [helion map](/helion/devplatform/als/user/reference/client-ref/#command-map) add these names
...
rename](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-node-attach) to
...
Locally, you can run
[dnsmasq](/helion/devplatform/als/user/reference/glossary/#term-dnsmasq) as a simple
...
The Application Lifecycle Service [micro cloud](/helion/devplatform/als/user/reference/glossary/#term-micro-cloud) runs with
...
More on  [NATS](/helion/devplatform/als/user/reference/glossary/#term-nats) communication
...
In this case it is necessary to tell [Polipo](/helion/devplatform/als/user/reference/glossary/#term-polipo) about the proxy so it knows how to handle this correctly.
...
Application Lifecycle Service caches all application dependencies that are downloaded by module managers that support the [HTTP\_PROXY](/helion/devplatform/als/user/reference/environment/#term-http-proxy) environment variable (e.g. pip, PyPM, PPM, NPM, etc). This is limited to 100MB of in-memory cache.
...
config using [kato config set](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-config). For example:
...
data can be found in the [Persistent Storage](/helion/devplatform/als/admin/best-practices/#bestpractices-persistent-storage)
section.
...
[VCAP\_SERVICES](/helion/devplatform/als/user/reference/environment/#term-vcap-services)
environment variable.

To tie external databases to Application Lifecycle Service as a data service, see the
examples in the [Adding System
Services](/helion/devplatform/als/admin/reference/add-service/#add-service) section.

###HTTPS & SSL {#https-ssl}

HTTPS mode provides access to the provisioned apps using wild card SSL
certificates through the router or
[Nginx](/helion/devplatform/als/user/reference/glossary/#term-nginx) web server.
...
install](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-op) command.
Usage:
...
-   [*helion quota configure*](/helion/devplatform/als/user/reference/client-ref/#command-quota-configure)
-   [*helion quota
    create*](/helion/devplatform/als/user/reference/client-ref/#command-quota-create)
-   [*helion quota
    delete*](/helion/devplatform/als/user/reference/client-ref/#command-quota-delete)
-   [*helion quota
    list*](/helion/devplatform/als/user/reference/client-ref/#command-quota-list)

Existing quota definitions can also be viewed and edited in the
Management Console [Quota Definitions settings](/helion/devplatform/als/admin/console/customize/#console-settings-quota-definitions)
...
Users (with our without *sudo* permissions) can install Ubuntu packages in application containers by requesting them in the requirements section of an application's *manifest.yml* file. The system allows package installation only from those repositories specified in the **Allowed Repos** list in the Management Console. 
 ...
 This list can also be viewed and modified at the command line using [kato config](/helion/devplatform/als/admin/reference/kato-ref/#kato-command-ref-config). For example, to view the 
 current list: 
...
Once a repository has been added to the list, **the GPG key must also be added** to the [Docker base image](/helion/devplatform/als/admin/server/docker/#modifying-or-updating-the-container-image) on each DEA (or the [Docker registry](/helion/devplatform/als/admin/server/docker/#creating-a-docker-registry) server if configured). 
...
**Warning**: Reconfiguring ALS to allow user applications to mount NFS partitions has serious security implications. See the [Privileged Containers](/helion/devplatform/als/admin/server/docker/#docker-privileged-containers) section for details.

By default, application containers are unable to mount external filesystems (other than the built-in [Filesystem Service](/helion/devplatform/als/user/services/filesystem/) via network protocols such as NFS.

If the system has been configured to use [privileged containers](/helion/devplatform/als/admin/server/docker/#docker-privileged-containers) and *sudo* permissions have been explicitly allowed in the quota, NFS partitions can be mounted in application containers using application configuration similar to the following manifest.yml excerpt:
requirements:

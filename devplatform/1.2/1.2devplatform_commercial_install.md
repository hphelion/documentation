...
1. Register at and download the package from the [HP Helion Development Platform](https://helion.hpwsportal.com/catalog.html#/Home/Show) (HDN). Verify that the package you download is 1.2.0.4 or later.
...
7. <a name="commandline"></a>Run this command to install the Helion Development Platform. <br /> By default the Username is *admin* and the Region is *regionOne*. Use the *admin tenant_id* from the overcloud. The parameter *location/ephemeralca-cacert.crt* can be found on the seed at */usr/local/share/ca-certificates/ephemeralca-cacert.crt*
	...
	**Note:** This installer will check to ensure that you are installing on a compatible version of Helion OpenStack. You will only be allowed to install Helion Development Platform if you meet a minimum Helion OpenStack version requirement (1.1.1); for update information, see <a href="/helion/openstack/1.1/updates/">HP Helion OpenStack 1.1: Update Procedure</a>. It is possible to override this check by passing the -x parameter to the <code>DevelopementPlatform_Enable.sh</code> script, as in the following example:

...
* Perform a fresh <a href="/helion/openstack/1.1/install/overview/">install</a> of Helion OpenStack version 1.1.1
...
- The installer requires the following packages. If they are not found, it will prompt you to install them.
	...
	* python-pip
	...
	These packages can be installed using <code>apt-get</code> on the Base node, but on the Seed node, you 'll need to use <code>pip</code> to install the following:
...
1. In the **Configure Services** panel locate the **Messaging** item in the Configure Services table and select **Download Service** and wait for the download to complete.
	...
	<img src="media/HDP_install_messaging.png"/>

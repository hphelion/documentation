...
5. Validate Keystone access on port 5000 using *curl*. Since Keystone access uses secure HTTP, you'll need to either use the certificate, or run *curl* with the <code>--insecure</code> switch to ignore SSL:
		...
		# Use one or the other of the following two lines to verify connectivity:
...
#### Step 1: Retrieve the ALS Service Update from HDN
...
Use the **Helion Development Platform UI** to check HDN for a new Service Update for ALS.

If there is a new Service Update for ALS, retrieve it from HDN. This will retrieve the ALS package from HDN and push it to a container in Swift within your local cloud.
...
Increase the Ports quota to at least 100 using the following command in the [Networking command line client](http://docs.openstack.org/cli-reference/content/neutronclient_commands.html): 
	...
	neutron quota-update --port 100 --tenant-id <tenant id>"

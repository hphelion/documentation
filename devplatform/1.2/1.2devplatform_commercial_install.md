...
HP Helion Development Platform version 1.2 currently contains the following components: 
<table>
<tr><th>Service</th><th>Version</th></tr>
<tr><td>Application Lifecycle Service (ALS)</td><td>1.2</td></tr>
<tr><td>Marketplace Service</td><td>1.1</td></tr>
<tr><td>Messaging Service</td><td>1.1</td></tr>
<tr><td>Database Service</td><td>1.1</td>
</table>
The following topics explain how to install and configure a new deployment of the HP Helion Development Platform version 1.2. For instructions on how to upgrade from version 1.1, see <a href="/helion/devplatform/1.2/upgrade/">Upgrading from HP Helion Development Platform 1.1</a>. There is also [Troubleshooting](/helion/devplatform/1.2/install/troubleshooting/) documentation to handle issues that may occur.
...
**Note:** Helion OpenStack version 1.1.1 is required for Helion Development Platform 1.2. Installing Helion Development Platform 1.2 on Helion OpenStack versions 1.1 or 1.0 is not supported. See <a href="/helion/openstack/1.1/updates/">HP Helion OpenStack 1.1: Update Procedure</a> for information on upgrading your Helion OpenStack installation.
...
- The HP Helion Development Platform installer (<code>DevelopmentPlatform_Setup.sh</code>) must be run on either the base or seed node of the Helion OpenStack environment. It is strongly recommended to run the installer  on the seed node. The seed node will already have the requisite key (/root/.ssh/id\_rsa) for access to the overcloud; if the installation is run on the base node, this key will need to be provided to the installer (see below).
...
2. Before you begin the installation, unpack the tar file. <span style='color:red'>(TODO: Verify this version number)</span>
...
5. Validate Keystone access on port 5000 using *curl*.
...
7. <a name="commandline"></a>Run this command to install the Helion Development Platform. <br /> By default the Username is *admin* and the Region is *regionOne*. Use the *admin tenant_id* from the overcloud. The parameter *location/ephemeralca-cacert.crt* can be found on the seed at */usr/local/share/ca-certificates/ephemeralca-cacert.crt*
	...
	**Note:** This installer will check to ensure that you are installing on a compatible version of Helion OpenStack. You will only be allowed to install Helion Development Platform if you meet a minimum Helion OpenStack version requirement (1.1.1); for update information, see <a href="/helion/openstack/1.1/updates/">HP Helion OpenStack 1.1: Update Procedure</a>. It is possible to override this check by passing the -x parameter to the <code>DevelopementPlatform_Setup.sh</code> script, as in the following example:
	...
	If the version check is not disabled, and the version of Helion OpenStack is not compatible, you will see the following error:
		...
		If you are sure that this install is correct you can override this check with the -x flag.
...
<tr><td><p>USERNAME</p></td><td><p>The username of a cloud admin that has rights to perform an ALS Service Update.</p></td></tr>
<tr><td><p>PASSWORD</p></td><td><p>The password of a cloud admin that has rights to perform an ALS Service Update.</p></td></tr>
</table>
...
2. Copy and paste the following commands into your terminal to perform the update:
		...
		export TE\_DATAFILE="/root/tripleo/ce_env.json" && source /root/tripleo/tripleo-incubator/overcloudrc
...
4. The installation is complete. Log back into the Horizon console (You may use a non-Admin account here.) Click on the **Marketplace** panel under the current Project to begin using the Marketplace Service. You may now install [Marketplace packages](/helion/devplatform/1.1/marketplace/#install).

...
The following steps describe how to download, install, and configure each of the Helion Development Platform services (Application Lifecycle Service [ALS], Messaging, Database, and Marketplace) using the Horizion interface.
...
#### Step 2: Initial Install of ALS Service

The Development Platform Installer is used to perform the initial installation of ALS.

It can also optionally update development platform user interface components. In order to do so, both of the following conditions must be true:

* This host must have network access to the cloud's **overcloud** controllers
* You must have an SSH private key that allows access to the **overcloud** controllers

If either of those conditions are not met, you can do either of the following:

* Run the following script from a host where the conditions are met (the seed node, for instance)
* Update user interface elements independently as part of service updates

The following script samples demonstrate use of the <code>DevelopmentPlatform_Setup.sh</code> script to show the help file and to perform the ALS installation: 

	# Show help for options available:
	./DevelopmentPlatform_Setup.sh -h
 
	# Explicitly set all options:
	./DevelopmentPlatform_Setup.sh \
		-k <path to key> \
		-r regionOne \
		-u admin \
		-p <password> \
...
#### Step 2: Update the ALS Service

##### Preparation

You will need the following information to run the ALS Service Update.

<table>
<tr><th><p>Identifier</p></th><th><p>Description</p></th></tr>
<tr><td><p>HOST_NODE_IP</p></td><td><p>The IP address of the host node for your Helion installation.</p></td></tr>
...
1. SSH to the seed node of your cloud:
		# SSH to the host node
		ssh USERNAME@HOST_NODE_IP
		...
		# SSH to seed node
		ssh SEED_NODE_IP
2. Copy and paste the following commands into your terminal to perform the update:
		# Ensure your environment has the proper credentials to perform the download from Swift
		export TE_DATAFILE="/root/tripleo/ce_env.json" && source /root/tripleo/tripleo-incubator/overcloudrc
		 
		# Download the Service Update from Swift
		swift download als-horizon-ui-installer
		if [ $? -ne 0 ]; then
		  echo "Unable to download the installer from Swift. This could mean:"
		  echo "- the Swift container doesn't exist"
		  echo "- the installer isn't in the Swift container"
		  echo "- unknown network error"
		else
		  # Source undercloud creds to allow script to discover Horizon nodes
		  source /root/tripleo/tripleo-incubator/undercloudrc
		  # Unzip the installer and kick it off
		  tar -zxvf als-horizon-ui-installer*
		  cd installer && ./install.sh /root/.ssh/id_rsa
		fi 

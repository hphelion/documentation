...
product: devplatform
title: "HP Helion 1.1 Development Platform: Best Practices "
...
The command will list the updates available. For example:
    ...
    1 updates available to be installed.
...
[scp](http://manpages.ubuntu.com/manpages/lucid/man1/scp.1.html) or
...
backup routine is left to the discretion of the Application Lifecycle Service administrator, but using [cron/crontab](http://manpages.ubuntu.com/manpages/oneiric/man1/crontab.1.html) is one simple way is to automate this. For example, you could create an entry like the following in the root user's crontab on the filesystem node:
...
in the [Nagios Core
Documentation](http://nagios.sourceforge.net/docs/3_0/toc.html)

...
6. **Running *kato patch install* fails to install patches on non-core nodes in ALS 1.1 clusters.** </br>This issue only applies to ALS clusters created using an installer image older than version 1.1.0.19. [Update the cluster](/helion/devplatform/1.1/install/troubleshooting/#katopatch) with the *cf-fix-kato-patch* command line tool.

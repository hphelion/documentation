---
layout: default
title: "HP Helion Cloud Version 12.12 to HP Helion Cloud Version 13.5: Data Transition Guide"
permalink: /migration-overview-reference/

---
#HP Helion Cloud Compute Version 12.12 to HP Helion Cloud Compute Version 13.5: Data Transition Guide#

This page provides you with information to help you transition an HP Helion Cloud Compute version 12.12 instance to an HP Helion Cloud Compute version 13.5 instance. 
...
The tasks require software tools such as the UNIX CLI for HP Helion Cloud, the Pythons Novaclient CLI, or the HP Helion Cloud Compute Service API. Before deciding which tools to use, make sure the tools are installed and configured on the local system and your instances, as required. 
...
4. [Create the HP Helion Cloud Compute version 13.5 instance](#createinstance).
...
8. [Review information on ongoing UNIX CLI for HP Helion Cloud support](#CLIdeprecation)
...
### Understanding what's new for HP Helion Cloud Compute version 13.5 ### {#VersionDiffs}
HP Helion Cloud Compute version 13.5 is based on the latest version of OpenStack (Havana), which expands functionality and enhances the current capabilities of the existing services. The sections below give a brief overview of what's new for version 13.5.
...
###Purging unnecessary data and files###
...
Before you create your first HP Helion Cloud Compute version 13.5 instance, you must generate a key pair for that instance. You can create a new key pair or use an existing version 12.12 public key to generate a new key pair. The new key pair can be used with any subsequent 13.5 instances you create. 
...
You create both keys at the same time using any of the following methods:
	
- [Using the classic management console](#keypairconsole) 
- [Using the API](#keypairapi) 
- [Using the UNIX CLI for HP Helion Cloud](#keypairunix)
...
In HP Helion Cloud Compute version 12.12, key pairs could be used only in a specific availability zone in the US West region. 
...
**Note**: In the classic management console, there is a 100-character limit in the **Key Name** field. To use a longer name, follow the [API steps below](#keypairapi) to create a key pair.

To generate a key pair using the classic management console:
...
7. Click **Download** to save the private key in a PEM file on the local system or copy the key pairs into a text editor and Save the file in the PEM format on the local system. 
	...
	If you copy the text, do not change the formatting of the text. Note the location where you save the file for later use.

	IMPORTANT: If you do not save the file or copy the text, you will not be able to recover the private key.
...
For more information on this API call, see [HP Helion Cloud Compute Service API Reference](https://docs.hpcloud.com/api/v13/compute/#createKeypair).


###Generating a Key Pair using the UNIX CLI for HP Helion Cloud### {#keypairunix}

There are two methods you can use to generate a key pair using the UNIX CLI for HP Helion Cloud. 
...
**Tip**: For the full list of UNIX CLI for HP Helion Cloud commands, see the [UNIX CLI for HP Helion Cloud Release Notes](http://docs.hpcloud.com/cli/unix).
...
To generate a key pair using the Python Novaclient commands.
	...
	**Important**: You must have the Python Novaclient installed on the instance before using the CLI commands. For more information, see [HP Helion Cloud Python Novaclient CLI Installation](https://docs.hpcloud.com/cli/nova/install).
...
**Note**: In HP Helion Cloud Compute version 13.5, you can specify both individual ingress and egress rules. 
...
You can create your own security groups or you can change the default group, as needed. There are different methods that you can use to manage security groups:
...
**Important**: When you add, modify, or delete a security group rule, the rule immediately goes into effect for all instances using that security group.

####Using CIDR to control traffic#### {#securitygroupcidr}

You can restrict access to your 13.5 instance by configuring security group rules using Classless Inter-Domain Routing (CIDR). CIDR specifies an IP address range using a combination of an IP address and its associated network mask. CIDR notation uses the following format:
...
5. Add or delete a rule as follows:

	**Note**: You cannot edit a rule. To change a rule, delete the existing rule and create a new rule.	

	- Add a new rule by specifying the following in the top row in the Rules table, and then click **Add** in the **Manage** column:
...
For information on using the API to create and configure security groups, see the [HP Helion Cloud Compute Service API Reference: Security Groups](https://docs.hpcloud.com/api/v13/compute/#4.1.12SecurityGroups).
...
1. Launch a command line window or UNIX shell on a system configured to access the availability zone where you want to create the new security group. The nova environment variables determine the region and availability zone you can access.
...
1. Launch a command line window or UNIX shell on a system configured to access the availability zone where the security group you want to modify is located. The nova environment variables determine the region and availability zone you can access.

2. Execute the following command to display a list of the security groups for the current instance.
...
4. Execute the following commands to add or remove the rules as needed.
...
Persistent instance storage, or HP Helion Cloud Block Storage, exists beyond the life of an instance. If you terminate the instance, any data stored on associated volumes is retained.
...
Use one of the following sections to create an ephemeral or persistent instance:

- [Create an ephemeral instance](#createephinstance)
- [Create a persistent instance](#createpersinstance)

After you create the HP Helion Cloud Compute version 13.5 instance, you can copy the data from your HP Helion Cloud Compute version 12.12 instance to your new 13.5 instance.
...
You can use HP Helion Cloud Compute to create on-instance (ephemeral) storage by using either the classic management console or by using Python Novaclient commands.

- [Using the classic management console](#createephinstanceconsole)
- [Using Python Novaclient](#createephinstancepyth)

**Note:** HP Helion Cloud Compute version 13.5 does not assign a public IP address automatically. To connect with the instance, you must assign a *floating* public IP address to the instance. A floating IP address is the same as the public IP in HP Helion Cloud Compute version 12.12. The following procedures contain steps to assign the floating IP to the new instance.
...
3. Use the following command to allocate a floating IP address that you can attach to the instance.
	...
	HP Helion Cloud Compute version 13.5 does not assign a public IP address automatically. To connect with the instance, you must assign a *floating* public IP address to the instance. 
...
####Using block mappings#### {#blockmapping}

A block device mapping defines the block devices to be attached to an instance. A block device mapping uses the format: 
...
Use either of the following tools to create a persistent HP Helion Cloud Block Storage version 13.5 instance.

- [Using the classic management console](#createperinstanceconsole)
- [Using Python Novaclient commands](#createperinstancepyth)	

**Note:** HP Helion Cloud Compute version 13.5 does not assign a public IP address automatically. To connect with the instance, you must assign a floating public IP address to the instance. The following procedures contain steps to assign the floating IP to the new instance.

For more information on floating IP addresses, see [Managing Your Floating IPs](https://community.hpcloud.com/article/managing-your-floating-ips-135). 

####Creating a persistent instance using the classic management console#### {#createperinstanceconsole}
...
6. Select the **Make bootable from image**. 
	...
	Three buttons and a drop-down menu appear. 
...
8. Select the specific image from the drop-down menu.
...
14. In the menu, click **Floating IPs**.

15. In the Floating IPs page, click **Allocate New IP**.

	A new floating IP address is added to the table on the page.

16. Select the new instance from the **Port** drop-down and click **Apply**. 
...
2. Execute the following command to identify an image to use with the bootable volume:
...
4. Confirm that the volume was created using the following command:
...
6. Create the instance using the following command:
...
8. Use the following command to allocate a floating IP address that you can attach to the instance.
	...
	HP Helion Cloud Compute version 13.5 does not assign a public IP address automatically. To connect with the instance, the instance requires a floating IP address.
...
9. Use the following command to attach the floating IP to an instance:
...
**Note:** When you connect to an HP Helion Cloud Compute version 13.5 instance, you must use the floating IP address you created for the instance. If you do not know the floating IP, you can find the address using the classic management console on the Server Details page for your server. 
...
1. Launch Windows Remote Desktop connection, for example: **Start > All Programs > Accessories > Remote Desktop Connection**. 
...
- [Using a Linux system](#connectlinuxterm)
...
After logging in, a system window displays, which indicates you are connected to the HP Helion Cloud Instance.
...
###Using SFTP to copy files between Windows instances### {#copydatasftp}

You can use any SFTP client to securely copy files between Windows instances.
...
####Using WinSCP to copy files to your Windows instance####  {#copydatasftpuse}

WinSCP is a free program available for Windows that enables you to copy files between attached systems.

To use WinSCP to copy files, either the 12.12 instance or the 13.5 instance must have an SFTP server configured and running and the other instance must have a properly-configured WinSCP client. The following procedure assumes the 12.12 instance is the server and the 13.5 instance is the client.
...
To copy files to your 13.5 instance:
...
3. Use WinSCP to log into the HP Helion Cloud version 12.12 instance using the fields on the **Session** page:
...
1. On the 13.5 instance, click **Applications > Utilities** to launch a terminal.
...
You can use the classic management console to create a block storage volume, and then copy the data on the 12.12 instance to the new 13.5 instance.
...
###Creating and attaching a block volume using the UNIX CLI for HP Helion Cloud tool### {#transdataunix}

You can use the UNIX CLI for HP Helion Cloud to create and attach a block volume on the 13.5 instance. Then, copy the data on the 12.12 instance to the new 13.5 instance.
...
You can use the HP Helion Cloud environment CLI in Windows PowerShell to create and attach a block volume on the 13.5 instance. Then, copy the data on the 12.12 instance to the new 13.5 instance.
...
5. You need to prepare your volume for use, to bring the volume on line. See [Preparing Your Block Storage Volume For Use](https://community.hpcloud.com/article/preparing-your-block-storage-volume-use).
...
<li>Our <a href="/release-notes/">release notes for version 13.5</a> of the HP Helion Cloud software</li>

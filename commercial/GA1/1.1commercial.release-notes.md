...
- After an update, the node removal procedure may not work.  CORE-2940 / DOCS 1068 this is an issue in progress.

- **Overcloud updates may fail because you cannot stop VSA VMs** <br /> Overcloud updates require that all VMs be stopped before an update can proceed. During an overcloud update, you may find that you are not able to stop the VSA VM (by issuing a `virsh stop` command). This causes the update to stop.   To fix this problem,
...
- Cinder cannot create volumes after a system update.   The workaround for this problem is [[big]] This is a work in progress. <!---CORE-2986 /  DOCS1065-->

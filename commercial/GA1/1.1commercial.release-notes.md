...
*  Testing has shown that attempting to create large numbers (thousands) of VMs at the rate of 6VMs/min might result in VM creation failures of up to 10%. This issue is being investigated by the HP Helion OpenStack team with a view to addressing in a future release. In the interim, if you see this behavior, we recommend lower rates of VM creation. <!-- VNETEE-53 DOCS-1050 -->
...
log onto the VSA to get access to the CMC Cliq CLI. Shut down the VSA.  If you do not know the IP/Username/Password of the VSA system, you can  find it in `/etc/cinder/cinder.conf` on controller0 on the overcloud.

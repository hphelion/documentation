...
4. Download the `ovsvapp-patch-*.zip` and extract the files. 
...
5. Edit the `ovs_vapp.ini` file in the downloaded `hp-ovsvapp` directory: 
	...
	For updates from 1.1 to 1.1.1, if you enter the path, the script will update OVSvApp agent and configure SSL support for rabbitmq communication. If you do not enter the path, the script configures SSL support for rabbitmq communication, but will not update the agent.
...
8. Enable agent monitoring, which is active by default, by setting the following varable in the `neutron.conf` file on all three overcloud controllers.

		enable_agent_monitor=False

9. Restart the Networking service (Neutron) server on all 3 overcloud controllers to take into effect:

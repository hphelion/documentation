...
-  Make sure that Python version is greater than 2.7.9 on the seed cloud host (the system from which the OVSvApp installation will be launched).
...
2. Install or update the Fabric python library (https://pypi.python.org/pypi/Fabric) using the following command:
	...
	If Fabric is not installed on the system, the command installs the library. If Fabric is already installed, the command updated to the latest version.

...
In the HP Helion OpenStack 1.1 release, HP recommends that you disable Single Root I/O Virtualization (SR-IOV) in the NIC BIOS on all hardware used for running HP Helion OpenStack.

Failure to disable SR-IOV will result in severe disruptions in network connectivity between overcloud systems.  With SR-IOV enabled, openvswitch will improperly learn port-to-MAC address mappings for tap devices used by overcloud services.  The MAC address for a tap device will be learned on both the physical interface (eth0) and the correct port in openvswitch, causing “flapping” in the openvswitch MAC table.  By disabling SR-IOV, this behavior is avoided.  Disabling virtual functions in the system kernel is not sufficient, SR-IOV must be disabled in the system BIOS.

...
This procedure is based on the [architecture diagrams](/helion/openstack/1.1/flexiblecontrol/overview/#deploy) from the the Flexible Control Plane Documentation.
...
**Note:** Some of the following steps are specific to the physical node where the seed VM runs. Do not use the following steps to install bare metal for nodes where the seed VM will not be running.

...
2.    Set up the remote hosts. You must set up each remote host identified in your vm-plan file.
	...
	f. Create the *overcloud-config.json* file with the following contents. For definitions of the variables used and suggested values refer to the list of available [environment variables](http://docs.hpcloud.com/helion/openstack/1.1/install/envars). 
...
Set the value of this variable so it has the same name as the interface that carries the neutron external traffic on your overcloud compute nodes. If you do not specify the value of this variable, the default value is `eth0`.
	...
	You may also need to set `hypervisor.public_interface_raw_device`.
...
For more information, see: [Add, remove and replace nodes](/helion/openstack/1.1/technical-overview/#add-remove-replace-nodes-jumplink-span).

...
We will use the id of the failed node in some of the later clean-up steps in [Clean up the environment](#cleanup). Back on the seed node, do the following.
...
Monitor status (it may take a few minutes for the status to change to UPDATE_COMPLETE).
...
We will use the id of the failed node in some of the later clean-up steps ([Clean up the environment](#cleanup)). Back on the seed node, do the following.
...
Increase the scale parameter back to its original value and redeploy the overcloud.
...
The final step is to remove the Nova service entries for the failed controller: This should be done on the new controller.
...
1. On the seed node, verify the logs in `/var/log/` directories, especially those in the `heat`, `nova` and `ironic` subdirectories respectively.
...
**Problem 1:** You see waiting on "os-svc-restart -n rabbitmq-server" message hanging for long time.
...
**Problem 2:** After adding back controller0, you see that controller1 or  controller is the only node in its cluster and is waiting a long time for the other node to join. 

**Solution:** Run the following command on the controllers to show cluster status:

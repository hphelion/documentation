...
When the script completes successfully, the stack will be available for use with the replacement controller. Continue to the section Clean up the environment after controller removal and replacement.
...
**Problem 1:** You may see a waiting on "os-svc-restart -n rabbitmq-server" message hanging for long time.
...
**Problem 2:** After adding back controller0, you see that controller1 or Management controller is the only node in its cluster and is waiting a long time for the other node to join. 

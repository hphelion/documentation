...
<<<<<<< HEAD:commercial/GA1/commercial.troubleshooting.install.md
# HP Helion OpenStack&#174; 1.1: Installation Troubleshooting

>>>>>>> 56ef44a2dadcf8d3c90b4b10ccd43aa11d2c6cf8:commercial/GA1/1.1commercial.troubleshooting.install.md

This topic provides possible solutions to known issues.

- [Set up Serial Communication to Enable Windows&reg; Debugging](#windebug)
...
## Set up Serial Communication to Enable Windows Debugging {#windebug}
Debugging between Windows instances may require setting up serial communication. Once serial communication has been established between the instances, which is usually over the COM2 port, the Windows Debug utility will function properly.

**Before you Begin**: Deploy a [debug host on the same compute host](http://docs.openstack.org/user-guide-admin/content/specify-host-to-boot-instances-on.html) as the target Windows instance. 

**Set Up Serial Communication**

1. Log on to the Compute hosts where the Windows instances (Target and debug hosts) are deployed.
2. Install the *socat* utility in the compute host. 
3. Get the serial source path of the target and the debug host VMs.
	...
	2. Look in the *filename.xml* file for the value of the **Source Path** for the target host. This value is found under the "Serial type='pty'" section. Example:
	...
	1. Repeat these steps to retrieve the **Source Path** value for the debug host. 
4.	Run the `socat` command to establish connection between the instances. In this example, the Source Path values were */dev/pts/4* and */dev/pts/5*. 
		...
		/usr/bin/socat /dev/pts/4 /dev/pts/5

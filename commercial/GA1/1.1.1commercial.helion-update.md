...
The Helion OpenStack v1.1.1 update scripts update Helion OpenStack v1.1 to version v1.1.1 and starts the update process of Helion Development Platform v1.1 to v1.2.  To complete the Helion Development Platform update to v1.2, refer to the Helion Development Platform documentation listed in [Updating Helion Development Platform](/helion/devplatform/1.2/upgrade/).
...
These procedures are described in the following sections. You must perform each of these three procedures in the order presented: first the seed host, then the undercloud, and finally, the overcoud.
...
If you have defined non-default IP ranges and other settings, source the `kvm-custom-ips.json` file before launching the update script (`update_sd.sh`) by entering:
...
1. To run the update script and copy the `kvm-customer-ips.json` file, enter:
...
If you have defined non-default IP ranges and other settings, source the `kvm-custom-ips.json` file before launching the update script (`update_sd.sh`) by entering:
	...
	source tripleo/tripleo-incubator/scripts/hp_ced_load_config.sh tripleo/configs/kvm-custom-ips.json 
<!-- copy note from seed host for HDP users -->
...
<!-- add note from seed host about HDP users not destroying their install -->
<!--
...
1. Create the new from-heat.conf file on the seed using the following command:
		...
		root@hLinux:~# cat > from-heat.conf  << EOF
		{{#stunnel}}
		{{#connect_host}}
...
After you have successfully updated the overcloud, there are some files that you should delete. To find and delete these files: 
...
If when you are running the overcloud update script and you see this failure message:
...
You can determine the source of the failure by entering:

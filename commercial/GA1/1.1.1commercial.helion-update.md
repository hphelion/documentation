...
If you are unsure of the version of Helion you are running, on any Helion OpenStack node, enter:
...
### Updating modified Heat templates {#templateupdate}

[[asked tom is this still applies now
...
1. Copy the new templates from the location where you unpacked the new kit to the original `/root/triple/tripleo-heat-templates` directory
	
    	cp -r /root/helion-update-1.1-to-1.1.1/tripleo/tripleo-heat-templates /root/tripleo/tripleo-heat-templates
    
	**Note**: if you have made modifications to your original heat templates, you will need to re-apply them to the new templates. See [[ xref for details. move from beginning?
...
1. Copy the new passthrough files from the location where you unpacked the new kit to the original `/root/tripleo/hp_passthrough` directory.
	...
	**Note**: if you have made modifications to your original passthrough files, you will need to re-apply them to the new passthrough files
...
1. Copy the new `my.cnf` image element from the undercloud to each of the Overcloud controller nodes. Log on to the seed and source the seed credentials. To do this, enter:
 		...
 		root@hLinux:~# . stackrc
    	root@hLinux:~# nova list | grep ctlplane | awk '{print $12}' | sed -e "s/.*=\\([0-9.]*\\).*/\1/"
    	root@hLinux:~# scp heat-admin@<undercloudip>:/usr/libexec/os-apply-config/templates/mnt/state/etc/mysql/my.cnf /tmp/my.cnf
    	source the undercloud creds and copy the file to each of the Overcloud controllers
   	 	for ip in `nova list | grep ctlplane | awk '{print $12}' | sed -e "s/.*=\\([0-9.]*\\).*/\1/"`; do
   	 	echo $ip;
...
1. Copy the new stunnel image element to `/usr/libexec/os-apply-config/templates/etc/stunnel/from-heat.conf` on each of the overcloud controllers
    for ip in `nova list | grep ctlplane | awk '{print $12}' | sed -e "s/.*=\\([0-9.]*\\).*/\1/"`; do
...
1. Run `update-overcloud` to apply the new templates and passthrough files by logging on to the seed as root and 
    cd to the original /root directory and running:
	...
	**Note**: If you defined any custom variables for the initial installation of the overcloud, you need to export them again before executing the hp_ced_installer.sh script. For example:
		...
		OVERCLOUD_NTP_SERVER=11.22.33.44 
...
1. Log onto each overcloud controller in turn and restart MySQL and stunnel. This ensures that they are listening on the correct ports. To restart MySQL, enter:
	...
	Check that mysql is now listening on port 3307 by entering:
	...
	**Note**: The Nova service-list will report that services are disabled. At this stage, this is expected behavior.

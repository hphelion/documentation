...
1. Verify that there is enough free space on the system to carry out the update. The actual space you need must support a copy of the following folders: 
...
1. Check the free space on the system by entering:
	...
	The available space cannot exceed the actual space required (that is, the output from the `du -hcs` command).

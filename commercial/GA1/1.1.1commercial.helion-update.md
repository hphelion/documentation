...
The Helion OpenStack 1.1.1 update scripts update Helion OpenStack 1.1 to version 1.1.1 and starts the update process of Helion Development Platform 1.1 to 1.2.  
...
To details on the Helion Development Platform update to 1.2, see [Updating Helion Development Platform](/helion/devplatform/1.2/upgrade/).
...
**Warning for Helion Development Platform users**: If you have defined non-default IP ranges and other settings, source the `kvm-custom-ip.json` file before launching the update script (`update_sd.sh`). Failure to do so could result in damage to your Helion Development Platform installation. For more information see [Environment Variables being ignored](#envvarsignored).

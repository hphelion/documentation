...
The Helion Developer Platform requires Helion OpenStack v1.1.1. However, Helion OpenStack v1.1.1 does not currently support updating the Helion Development Platform from v1.1 to v1.2.

<!--At this time, the Helion Developer Platform requires Helion Open Stack v1.1.1. Upgrading Helion Development Platform from v1.1 to v1.2 is currently not supported. -->


**Note**: You can also deploy the Helion OpenStack v1.1.1 release from scratch. For those instructions, please see [Installing Helion OpenStack](/helion/openstack/1.1/install/overview/).
...
1. Your Helion OpenStack undercloud is now updated. Confirm that update was successful by examining the ` /var/log/ansible/ansible.log`.  This report should not list any unreachable nodes:
...
1. Update the playbook by entering:
		...
		root@hLinux:~# cat > ~/helion-update-1.1-to-<version>/tripleo/helion-update/tripleo-ansible/playbooks/step_run_occ.yml << EOF
		...
		- name: "Setting fact to bypass os-collect-config if 
		   applicable"

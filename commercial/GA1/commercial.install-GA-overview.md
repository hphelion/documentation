...
* Users and client software must be able to verify the TLS certificates used on the Helion APIs and web interface, so the deployer must make the CA certificate available to users. For more information see [HP Helion OpenStack TLS Support](/helion/openstack/tls/). 

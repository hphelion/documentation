...
After the update, the Overcloud controller should be back online without any errors. You should be able to  SSH to the Nova computer from the seed host. [[resume  
Should be possible to open horizon    
Should be possible to execute lifecycle operations with any instance already deployed in the OverCloud      
The node should re-join the MySQL Cluster        
HA Proxy should show the servers as online    
The Instances deployed before update are still up/accessible           
Is possible to deploy new instances (check if will be possible to ping/ssh)      
 
Check in icinga if all the monitor for the update node are green 
- http://undercloudip/icinga
- Credential: icingaadmin/icingaadmin
 
Enable HA-Proxy status page and check if the all the services from the node are green (up/running)
- SSH to the Overcloud Management controller
- Edit the file "/etc/haproxy/haproxy.cfg"
- Add in the end of the file "mode http" (in the listen haproxy.stats :1993 section)
- Restart HA Proxy (service haproxy restart)
- Open the status page and check all the services, URL: http://managementcontroller.ip:1993
                
Commands that need to be working  after update:                
keystone user-list                
nova list               
neutron net-list     
glance image-list  
...
Networks and Bridge Configuration:      
Check if all networks still exist after update, same as before (ifconfig)             
Check if all bridges and ports still exist after update, same as before (ovs-vsctl show)   
 
Validations Post update: (Compute Node)
Items to be validated after update:
The Instances deployed before update are still up/accessible
Should be possible to SSH to the nove from seed
Is possible to deploy new instances (check if will be possible to ping/ssh)
...
Validations Post update: (Swift)
Items to be validated after update:
Check if the Images still there
Should be possible to SSH to the nove from seed
Check if is possible to load new images
Check if is possible to deploy new instances with a new image
 
Commands that need to be working  after update:
os-refresh-config
 
Validations Post update: (VSA)
Items to be validated after update:
Check if the node join the cluster back (CMC)
Should be possible to SSH to the nove from seed
Check if the volumes still present
Check if is possible to create new volumes
Check if is possible to delete old volumes
Check if is possible to attach/detach volumes
 
Check the version of the glance image used to update, if the image id is the same
SSH to the UnderCloud node
glance image-list
nova list
nova show <instance id>
Compare the Image ID from “glance with the image name/ID from “nova show”

...
If you are unsure of the version of Helion you are running, on any HOS node, enter:
...
To prepare the seed host for update, verify that the following files exist in the seed node.
...
1. Log into the seed host as root and create an update directory by entering:
...
1. Copy the seed update script from the downloaded tarball on the seed to the seed host by entering:
...
2.	Download the update tarball onto the seed host.
...
1. The restoration command will take some time, but once completed you should be able to login to the seed host.  To verify that the node is in working status, enter:

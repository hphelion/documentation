...
If you have modified your Orchestration (Heat) template, then you will need to update the new templates that are provided with the installer with these modifications. If they have not made any changes to the original template, then they should be safe to execute `./tripleo/tripleo-incubator/scripts/hp_ced_installer.sh --update-undercloud` and `./tripleo/tripleo-incubator/scripts/hp_ced_installer.sh --update-overcloud` to update the HEAT templates. 

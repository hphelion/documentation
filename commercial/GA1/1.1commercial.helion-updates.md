...
This Helion OpenStack (HOS) version 1.1.1 update procedure supports Helion Development Platform (HDP) version 1.2 and only this HDP version. For users who have HOS 1.1 and HDP 1.1, when you update to HOS 1.1.1, you also need to update to HDP 1.2.  
...
This procedure involves downloading the script tarball on a seed VM and then copying that onto your seed host.
 
To prepare the seed host for update, verify that the following files exist in the seed VM.
...
1. Download the update tarball onto the seed VM by entering:
...
The root disk usage cannot exceed 40-45%.  If you need to free up space, you can safely remove the `helion_ee_1.X.tgz` tarball that you just extracted.
...
2. Copy the seed update script from the downloaded tarball on the seed VM to the seed host by entering:



    	scp root@<seed-vm-ip>:/root/helion-update-1.1-to-1.X/tripleo/helion-update/seed_update/update_sd.sh .

2. Run the seed update script `update_sd.sh` from the seed host. Make sure you run `update_sd.sh` from the update directory you created: `/root/helion-update-1.1-to-1.X)` using the seed host's  `tripleo` root directory (`/root/tripleo`) and remote update root   (extracted updated tarball directory on the seed VM) as arguments.

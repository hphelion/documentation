...
Deploy the complete solution (Seed/UnderCloud/Overcloud). You do not need to populate the environment.
...
The procedure will guide you through updating the Seed host using `update_sd.sh`.

To update the Seed host:

1. Log into the Seed host by entering:


    	ssh root@<seed-ip>

	Make sure that no folders from previous update attempts exist in the `/root` folder.

1. Remove any update folders and contents in the `/root` folder using the `rm` command.
1. Download the update tarball on the Seed host by entering:
...
1. Log into the Seed host machine as root and create an update directory by entering:
...
1. Copy the Seed update script from the downloaded tarball on the Seed to the Seed host machine by entering:



    	scp root@<seed-ip>:/root/helion-update-1.1-to-1.X/tripleo/helion-update/seed_update/update_sd.sh .

1. Run the Seed update script `update_sd.sh` from the Seed host. Make sure you run `update_sd.sh` from the update directory you created: `/root/helion-update-1.1-to-1.X)` using the Seed host's `tripleo` root directory (`/root/tripleo`) and remote update root(extracted updated tarball directory on the seed) as arguments.
...
2. To purge the known_hosts entrys on the Seed host machine, enter:
...
The likely reason for this failure is the installer update of the Seed image has failed as the configuration that was used during the install was not available or passed on to the installer to perform the Seed update operation.  As a result, the installer has indicated that it has failed, although the Seed has likely rebooted without issue.
...
1. Identify the following:
	...
	- Expected IP address of the Seed.
...
1. Verify that you can ping the IP address of the Seed by entering:
...
1. The restoration command will take some time, but once completed you should be able to login to the Seed.  To verify that the node is in working status, enter:
...
## Ironic fails because nodes are not in maintenance mode ##

During an update, nodes must be in maintenance mode or Ironic returns an error message such as the following:
...
If the command `ironic node-set-maintenance` fails to properly change nodes to maintenance mode, you must either update the database directly using:

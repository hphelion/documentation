...
3. Use the following command to specifies that test runs on the unit test directory

		OS_TESTPATH=./tempest/{service} run-tempest

	Where `{service}` is the name of the service, such as `api`, `compute`, `identity`, or `network`.

	For example, execute the following command to run the Tempest test against the APIs:
...
3. Edit the `[service_available]` section of the `/opt/stack/tempest/etc/tempest.conf.sample` file:

	Set a line to `true` to test that service or `false` to not test that service.
...
4. Edit any of the `*_feature_enabled` sections to enable or  disabled tests which require a specific feature of a specific service. 
...
Whitelisting a test allows the test to run. Blacklisting a test prevents the list from being run. 

In the `tests2skip.py` file use a `+`character to whitelist a test or use a `-` character to blacklist a test: 

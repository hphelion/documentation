...
5.	After the prerequisites are installed, you should configure a local NTP server on the laptop. The NTP server is required for the undercloud and overcloud installation. If your laptop does not use an external NTP server, you must configure a local NTP server. To do this, follow these steps:
...
6.	After you create the `vm_plan.csv` file. The following example `vm_plan.csv` contains information for eight virtual machines with roles assigned, memory allocated, and the number of assigned vCPUs.
		...
		,,,,4,6144,,OvercloudCompute
**Note**: If your system is limited to 32 GB of memory you should use the values provided in the example above. Not of all of these virtual machines will then be created when using 32 GB of memory.
...
As described above, if your laptop or demo system has insufficient memory for a full configuration, you can configure a Virtual Installation with a restricted configuration.
	...
	f. When running with limited hardware resources it may be necessary to increase the default time-outs:
		...
		`export OVERCLOUD_STACK_TIMEOUT=240` <br>
...
5. If a second computer to access the web interface is available:
	...
	6.	Open the browser on another machine that can reach your demo laptop and enter: <br />

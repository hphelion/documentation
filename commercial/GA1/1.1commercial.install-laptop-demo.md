...
5. If a second computer to access the web interface is available:
	1. Create a tunnel from the Horizon console to the laptop: 
		...
		where the overcloud IP is the one printed on screen at the end of the installation process as in point 9 of Second stage of the Helion install (Under/Overcloud nodes install) section and laptop IP is the IP assigned to the card the laptop is connected to the network with.

...
3. Edit the scale counts in JSON environment variables file ( for example: `kvm-custom-ips.json`) that was used during the initial installation to define the appropriate scale number:
 	...
 	**Note**: While deploying a scale-out **proxy** node ensure that the value of `so_swift_proxy_scale` is appended. For example: If the initial number of proxy servers were 2 and later you added 2 more proxy servers then the total number of proxy servers must be 4. The initially added proxy server should always get appended with the newly added object server. Also, while deploying a scale-out **proxy** node ensure that the value of `so_swift_storage_scale` is not modified. The value of `so_swift_storage_scale` should remain unchanged.

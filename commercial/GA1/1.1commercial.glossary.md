...
## M
###MoM
Monitoring of Monasca. Monasca can monitor a wide range of Helion services, including itself.
###Monasca
The monitoring as a service solution introduced in Helion 2.0. Monasca (meaning monitoring at scale) is an open source solution that integrates with OpenStack. It is fault-tolerant, multi-tenant, scalable, and high performing.
...
Refers to the Seed Host.

### Seed Host<a name="Seed-Host"></a> ###

Also known as the Seed Cloud Host, or Seed Node, refers to the physical server where the Seed VM is running. 

### Seed Node ###

Refers to the Seed Host (Seed Cloud Host).

### Seed VM<a name="Seed-Cloud"></a> ###
A bootable image that is deployed into a virtual machine (VM). The seed VM is a [Kernel-based Virtual Machine](#KVM). This running hLinux VM contains the minimal Seed Cloud. This is the minimum that OpenStack needs to allow Ironic to install the Undercloud. See also [undercloud](#Undercloud).

Also referred to simply as the Seed. See also [undercloud](#Undercloud) and [overcloud](#Overcloud). 

...
[See version 1.0 of this page](/helion/openstack/install/vsa/)
...
<!-- removal of steps 1-5 suggested by Tom Hancock. commented out-->
...
6. Edit the `/root/baremetal.csv file` in seed cloud with the details of the newly added node. Make sure to add the role of the node where you see &lt;role&gt; in the baremetal.csv instructions [that can be found here](/helion/openstack/1.1/install/csv/)).

8. <!---7. Edit the `/root/tripleo/configs/kvm-custom-ips.json` and update the value for `vsa_scale` or `vsa_ao_scale` appropriately.-->

...
Follow this procedure to configure HP Helion OpenStack to use customer-provided certificates for all endpoints.


The following steps will explain how to split up the existing load balancer to terminate SSL connections with a client provided server certificate and re-encrypt to service backends served by stunnel proxies. Here you will also add the client-provided trust chain to the ECA trust file. Note that while this eliminates the need to reconfigure serices to use the certificate you as the customer provide, services may need to be restarted.
...
The services use the endpoints to talk to other services and they'll fail validation because they're designed to find the ECA certificate from a file location. They need to be told to trust the client provided server certificate. Ideally, if the cert was signed by a well-known CA, this would not be an issue. However,  different libraries behave differently, and in the presence of venvs, we decided to hardcode the cacert location in 1.1.1. Therefore, the suggested procedure is to have both public CA signed cert and internal CA such as HPIT signed certs in a single file.

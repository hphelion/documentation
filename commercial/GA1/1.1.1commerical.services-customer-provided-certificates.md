...
This topic explains how to split up the existing load balancer to terminate SSL connections with a client provided server certificate and re-encrypt to service backends served by stunnel proxies. In addition, we add the client-provided trust chain to the ECA trust file. This is so that none of the services need reconfiguring. However, they may need to be restarted.

...
The following steps will explain how to split up the existing load balancer to terminate SSL connections with a client provided server certificate and re-encrypt traffic to service back ends served by stunnel proxies. Here you will also add the client-provided trust chain to the ECA trust file. Note that while this eliminates the need to reconfigure serices to use the certificate you as the customer provide, services may need to be restarted.
...
The services use the endpoints to talk to other services and they'll fail validation because they're designed to find the ECA certificate from a file location. They need to be told to trust the client provided server certificate. Because the cacert location has been hard coded in HP Helion OpenStack 1.1.1,  the suggested procedure is to have both public CA signed cert and internal CA-signed certificates in a single file.

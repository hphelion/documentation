...
The Cinder volume service runs only on Controller0, but the Icinga monitor for the Cinder volume service runs on all three Helion controller nodes (Controller0-2). This causes Icinga to send a warning that the Cinder volume service is not running on Controller1 and Controller2. This is a false alarm and can be ignored. 

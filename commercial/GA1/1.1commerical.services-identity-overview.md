...
Roles are created and assigned by the Identity service to either specific UserIDs or Groups. The function of the role itself is defined by each separate OpenStack service. More information on Roles can be found in the [User management section](http://docs.openstack.org/admin-guide-cloud/content/keystone-user-management.html) of the OpenStack Cloud Administrator Guide.

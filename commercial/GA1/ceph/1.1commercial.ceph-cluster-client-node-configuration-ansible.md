...
<p style="font-size: small;"> <a href=" /helion/openstack/1.1/commercial.ceph-automated-install/">&#9664; PREV</a> | <a href=" /helion/openstack/1.1/services/ceph/">&#9650; UP</a> | <a href=" /helion/openstack/1.1/ceph-monitoring/">NEXT &#9654;</a> </p>


# HP Helion OpenStack&#174; 1.1 and 1.1.1: Configuring Ceph Cluster and Client Nodes using Ansible Playbooks
...
2. Copy and untar the `helion-ceph.tar` file on one of the provisioned nodes identified as `ceph-admin` under `/helion-ceph/ansible-playbooks`. If `/helion-ceph/` does not exist, then create a directory named `/helion-ceph/`.
...
4. Once the nodes are provisioned (for more information on nodes provisioning, refer [Running the Provisioning Tool]( /helion/openstack/1.1/commercial.ceph-automated-install/#running-provision-tool)), create the following configuration files under `/helion-ceph/cephconfigureation/ansible-playbooks/` on the `ceph-admin` node 
	...
	**user**: used to login to Ceph nodes for performing the configuration. In this case it is **hlinux**. (**hlinux** refers to HP Linux for HP Helion OpenStack.)<br>
...
5. Edit the `group_vars` as per your set up:
	...
	**Note**: The `fsid` (File System ID) is a unique identifier for the cluster. Execute `uuidgen` command to generate `fsid`.
	...
	<!-- this is not required as we have already generated a keypair 6. Generate the `cephadmin.pem` key from the Undercloud node by entering:
...
9. If journaling is enabled, in `/group_vars/ceph-cluster`, run following scripts from the admin node from `helion-ceph/ansible-playbooks/`
		...
		sudo ./createjournalpartitions <cephadmin keypair>
...
10. Execute the following command.
		...
		sudo ./bootstrap.sh <cephadmin keypair>

...
2. Once the nodes are provisioned (for more information on nodes provisioning, refer [Running the Provisioning Tool]( /helion/openstack/1.1/commercial.ceph-automated-install/)), create the following configuration files under `/helion-ceph/cephconfigureation/ansible-playbooks/` on the `ceph-admin` node 
...
3. Edit the `group_vars` as per your set up:
		...
		secretuuid: 457eb676-33da-42ec-9a8c-9293d545c337 # This the UUID that will be used to setup the helion nodes. Change this prio to running the ceph-client and ceph-admin roles, if you wish to newly generated UUID. The same UUID will work too.
...
4. Generate the `cephadmin.pem` key from the Undercloud node by entering:

		nova keypair-add cephadmin > cephadmin.pem

5. Copy the  `cephadmin.pem` key file to the Seed node.
5. Copy the public key of heat-admin to enable SSH from the `ceph-admin` node to Helion nodes by entering:

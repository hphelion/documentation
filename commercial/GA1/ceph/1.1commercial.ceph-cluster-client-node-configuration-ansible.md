...
**<Heading should be present here. please check with SME and insert the heading> Paul**

Download the `cephconfiguration.tar` (**from where to download? -Paul**). Untar and copy the `cephconfiguration.tar` on one of the provisioned node identified as `ceph-admin` under `/helion-ceph/`. If `/helion-ceph/` does not exist then create a directory named `/helion-ceph/`.

1. Ensure the client setup scripts tar ball also contains the ceph debs for installing on Helion nodes have been copied under `/helion-ceph/ansible-playbooks/roles/ceph-client/files`.
 
For example: `/helion-ceph/ansible-playbooks/roles/ceph-client/files/ceph_client_setup-0.80.7_h1.1.fix6_newdebs.tar`

2. Once the nodes are provisioned create the following configuration files under `/helion-ceph/ansible-playbooks/` on `ceph-admin` node 
	...
	**node ip**: address used by configuration process to setup `ceph-cluster`. This can either be a pxe, management or some other network.<br>
	**ceph-identifier**: Unique name to identify an osd, mon, admin or rados. This will be used by bootstrap script to generate ansible metadata (Note that this can be different from actial node name).<br>
	...
	**user**: used to login to cephnodes for performing the configuration. <br>
	...
	**osd-disk-path**: disk to be used for osd storage (This attribute exists only for osd type node) . This will be an entire physical disk to be used as data disk. For example: `/dev/sda` or it can be a partition on a larg disk. For example: `/dev/sda5` <br>
	**journal-partition-type**: partition type to be used while formatting the disk (SSD) for journaling (This attribute exists only for osd type node). For journaling make sure entire disk is specified. Eg. `/dev/sde`. Please do not use partition for journaling.eg. `/dev/sde1` is not allowed.<br>

	**journal-disk-path**:disk to be used for journaling (This attribute exists only for osd type node).
...
3. Edit the `group_vars` as per your setup:
...
5. Copy the public key of heat-admin to enable ssh from `ceph-admin` node to helion nodes:
...
5. If journaling is enabled, in `/group_vars/ceph-cluster`, run following scripts from admin node from `helion-ceph/ansible-playbooks/`
	...
	**Note** - Before running this script make sure that no partition numbers are associated with <journal-disk-path> in `cephcluster.csv`. This script will append the partition numbers ones it is executed successfully.
...
6. Execute the following command.
	...
	This script creates ansible metadata from `cephcluster.csv` manifest file, configures and `/etc/hosts` file on ceph nodes.

	After the bootstrap is run verify the `hosts` file and `host_vars/*` files are generated.
...
Ensure that 3 keys are present or copied from respective nodes to admin node under `ansible-playbook` folder.

* `cephadmin.pem` key is copied (from undercloud) to `ansible-playbooks` folder.
* `seed-id_rsa.private` copied from seed to `ansible-playbooks` folder.
* `seed-id_rsa.public` copied from seed to `ansible-playbooks` folder.
 
**Note**: All these 3 key ring permission should be set to **sudo chmod 600 <key>**
...
###Ceph is deployed in 3 different stages

You can deploy ceph in three different stages.

1. Setup Monitors and OSD nodes.
2. Ceph Integration on Helion nodes
3. Setup Rados Gateway nodes in Ceph cluster

Execute the following command to deploy ceph in three different stages:

I. Setup Monitors and OSD nodes using the following command. 
...
II. Ceph Integration on Helion nodes[Controller and Compute nodes to support Ceph backend]. 
...
III. Setup Rados Gateway nodes in Ceph cluster using the following command 

		ansible-playbook –i hosts rados_setup.yml


**Sync of ceph.conf to ceph nodes, if any manual changes are made**

If any changes are made to `ceph.conf` file on any node manually, copy `ceph.conf` file as `ceph_master.conf` from that node to ceph admin node `/helion-ceph/ansible-playbooks/roles/ceph-mon-master/files/ceph_master.conf`.

Run the following command to get `ceph.conf` file synced with all the ceph cluster and helion client nodes.
...
* Provision the new node with ceph-cluster image using node-provisioning tool as mentioned above from seed. We need to have 3 monitor nodes.
...
**Sample Baremetal.csv**
 ...
 Once the node is up , from admin node modify /helion-ceph/ansible-playbooks/cephcluster.csv with the new mon node.
...
Verify, if you are seeing 3 monitors with `sudo ceph –w` command.

For ntp related warnings check the following section on how to sync all the servers in ceph cluser with ntp server.

####Add OSD {#add-osd}

* Provision the new node with ceph-cluster image using node-provisioning tool as mentioned above from seed.
...
Example: Have added 2 OSD nodes and under each node will have 4 OSD deamons.
...
* If journaling is enabled, in `/group_vars/ceph-cluster`, run following scripts from admin node from `helion-ceph/ansible-playbooks/`
...
* Run Ansible playbook to add the new OSD nodes

		 ansible-playbook –i hosts osd_mon_setup.yml

Verify if you are seeing newly added OSD nodes and daemons from `sudo ceph –w` command.

###Heartbeat Monitoring Tool (Optional) {#heartbeat-monitoring}

Heartbeat tool is written in Python and it is an open ended tool so other 3rd party service and status checks can be added or extended. This tool is run on the seed node from the HP Helion setup. This tool continuously queries for HP Helion OpenStack services and provide notifications to the pre-defined administrator when conf file mismatch or when any of the specified services in conf file are down. The Configuration service automatically reapplies the known good conf files when the checksum doesn't match, and restarts the associated services specified in the conf file and triggers the validation script to do a minimal check for that service. Even if the conf files are overwritten when the checksum doesn't match, a backup of existing file from current folder with [filename].timestamp is also saved in the backup folder so the admin can anytime will have the option to verify the changes.
...
These tarballs can be obtained from following location:
...
4. Ensure ceph health and status to be OK.
...
4. XFS fragmentation is useful to monitor and correct.

		Fragmentation on /dev/sdb1 - osd3
		actual 22722, ideal 22557, fragmentation factor 0.73%

5. Depending on workloads, further Ceph tuning parameters are:
...
* Create sample glance RAW image on any controller node as below. Raw data format is to be used with RBD for instant image snapshots and protection. For more details, refer  to [http://ceph.com/docs/master/rbd/qemu-rbd/?highlight=raw](http://ceph.com/docs/master/rbd/qemu-rbd/?highlight=raw)

* Conversion tool like `qemu-img` is used to convert from one image format to another. <br>
For example:
...
* Ensure uploaded glance image is available in the Horizon UI and is correctly stored in the appropriate pool in Ceph.
...
* Enable logging in `glance-api.conf` if problem is encountered in any of the above steps. Restart Glance services and re-run problem step. Gather Glance debug logs in `/var/log/glance` directory and contact HP support team for inputs.

###Ceph Glance Clone Copy On Write {#glance-clone-copy}

* Commands to create glance image using glance image-create command.
...
* Create cinder volume from glance image created above on any controller node

		cinder create –image-id <glance image id> --display-name RVol 2

<img src="media/ceph-ansible-playbooks-glance-clone1.png"/)>

* Ensure cinder volume created is available in rbd pool

		rbd ls –l <cinder pool name>
		cinder list
<img src="media/ceph-ansible-playbooks-glance-clone2.png"/)>

* Track clones to demonstrate copy-on-write feature by first listing snapshots of glance image, and then listing the children of the snapshot
...
* Enable logging in `glance-api.conf` and `cinder.conf` if problem is encountered in any of the above steps. Restart Glance and Cinder services and re-run problem step. Gather Glance debug logs in `/var/log/glance` directory and cinder debug logs in `/var/log/upstart` directory and contact HP support team for inputs.
...
7. Attach the cinder volume to a nova instance.

		nova volume-attach <instance id> <volume ID>

8. Select the instance and provide the device name.

		nova volume-attach <server> <volume> <device>

####Mount the Volume and Copy a new image

Now, mount the volume and copy the new image file from the VM. Perform the following steps.
...
1. Login to overcloud Horizon.
2. From the left panel, click the **Projects** tab and then **Instances** to view the list of  instances.
3. Click on the **Instance Name** to view the instance console log. (**please verify this step with SMEs**)



Perform the following:

1. Create the instance with RBD backend. (**it would be great if we can get the command**)

	<img src="media/ceph-ansible-nova1.png"/)>

2. List all the nova instances
...
1. Execute the following command to attach volume to a nova instance
...
Make GET request to gateway server

* FQDN or IP address of gateway server is used. 
...
* If there is error with Apache2 or FastCGI, look for debug logs in /var/log/apache2/error.log. Changing permissions accordingly on `/var/www directory` or `/var/www/s3gw.fcgi` file may fix the problem.
...
* S3 users and swifts users need to have access and secret keys to enable end users to interact with gateway instance. Execute the following command to create the access and secret key for s3User:
...
* Gateway instance and swift users can be verified on gateway node or Ceph client using Swift Client by making swift v1.0 requests

...
2. Once the nodes are provisioned (for more information on nodes provisioning, refer [Running the Provisioning Tool]( /helion/openstack/1.1/commercial.ceph-automated-install/#running-provision-tool)), create the following configuration files under `/helion-ceph/cephconfigureation/ansible-playbooks/` on the `ceph-admin` node 

...
2. Execute radosgw service in debug mode to ensure that there are no obvious errors in logs on both gateway nodes

		/usr/bin/radosgw -d -c /etc/ceph/ceph.conf --debug-rgw 20 --rgw-socket-path=/var/run/ceph/ceph.radosgw.gateway.fastcgi.sock

3. From controller node, make Swift v1 request like below. IP address used will be Helion VIP address followed by radosgw service port. Ensure s3 and swift user are created using radosgw-admin commands. Output should list container if any. 

		swift --insecure -V 1.0 -A https://192.0.2.x:10080/auth/v1.0 -U s3User:swiftUser -K abc list

4. From controller node, make Swift v2 request using keystone. Ceph Object Gateway’s user:subuser tuple maps to the tenant:user tuple expected by Swift. IP address used will be Helion VIP address followed by keystone service port. Here, admin credentials are considered. Output should list container if any.
